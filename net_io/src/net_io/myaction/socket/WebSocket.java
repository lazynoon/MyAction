...
public class WebSocket  {
	private _WebSocket socket = new _WebSocket(this);
	...
	private class _WebSocket extends StreamSocket {
		private WebSocket that;
		private _WebSocket(WebSocket that) {
			this.that = that;
		}
				...
				if(position >= 4) {
					...
					if(b1 == '\r' && b2 == '\n' && b3 == '\r' && b4 == '\n') { //找到head结束符
						...
						if(pass && that.onCreateInbound(conn.sid, head)) { //允许连接
							...
							accept.append("\r\n");
							accept.append("\r\n");
							super.send(conn.channel, ByteArray.wrap(accept.toString(), "UTF-8"));
						} else {
							pass = false;
						}
						if(!pass) {
