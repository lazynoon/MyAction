...
public class AppConfig {
	/** MyAction 框架版本 **/
	public static final String MYACTION_VERSION = "1.1.1";
	...
	// 默认连接超时时间（单位：ms）
	private static long defaultConnectTimeout = 30 * StatNIO.ONE_THOUSAND_LONG;

	public static volatile long requestCount = 0;
	public static volatile long processCount = 0;
	public static int concurrent = 0;

	private static ArrayList<String> configNames = new ArrayList<String>();
	...
	public static void setProperty(String name, String value) {
		if(name == null) {
			throw new IllegalArgumentException("setProperty name is null");
		}
		if(value == null) {
			throw new IllegalArgumentException("setProperty value is null");
		}
		name = name.trim();
		if(name.length() == 0) {
			throw new IllegalArgumentException("setProperty name is empty");
		}
		String lowerName = name.toLowerCase();
		if(!configMap.containsKey(lowerName)) {
			configNames.add(name);
		}
		configMap.put(lowerName, value);
	}
