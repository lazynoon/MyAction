...
public class MyActionSocket extends PacketSocket {
	...
	public void onReceive(final NetChannel channel, final BaseMsg msg) throws Exception {
		...
		if(msgID == SocketRequest.MSG_ID)  { //请求类型
			...
			producerPool.execute(new SocketActionProcessor(channel, cmdMsg));
			...
			int reqID = cmdMsg.getRequestID();
			Reader reader = readerMap.get(reqID);
			if(reader != null) {
				reader.callback(cmdMsg);
			} else {
				channel.putResponse(reqID, cmdMsg);
			}
		} else {
	...
	public static interface Reader {
		...
		public void callback(CommandMsg msg) ;
	}
